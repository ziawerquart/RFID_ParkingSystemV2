...
{
    BYTES_PER_LINE = 16;
    // BYTES_PER_LINE bytes, 2 chars per byte, and BYTES_PER_LINE - 1 spaces
    HEXCHARS_IN_LINE = BYTES_PER_LINE * 2 + BYTES_PER_LINE - 1;//47;
    // Indicate that the space between the address area and hex area
    GAP_ADR_HEX = 10;
    // Indicate that the space between the hex area and the ascii area
    GAP_HEX_ASCII = 16;
    ...
    _cursorTimer.start();
    _xData.setBytesPerLine(BYTES_PER_LINE);
}
...
int QHexEditPrivate::bytesPerLine() const
{
    return BYTES_PER_LINE;
}

void QHexEditPrivate::setBytesperLine(int bpl)
{
    if(bpl > 0)
    {
        BYTES_PER_LINE = bpl;
        HEXCHARS_IN_LINE = BYTES_PER_LINE * 2 + BYTES_PER_LINE - 1;//47;
        _xData.setBytesPerLine(bpl);
        adjust();
    }
}
...
void QHexEditPrivate::adjust()
{
    ...
    // MASKED BY LIJIAN
//    setMinimumHeight((((_xData.size()+15)/16/* + 1*/) * _charHeight) + 5);
    setMinimumHeight((((_xData.size() + (BYTES_PER_LINE - 1)) / BYTES_PER_LINE) * _charHeight) + 5);
    if(_asciiArea)
